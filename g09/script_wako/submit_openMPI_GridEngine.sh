...
mpirun -machinefile $TMP/machines -np 1 -npernode 1 --map-by slot:pe=${QM_NUM_THREADS} $GENESIS genesis_qmmm_min.inp >& genesis_qmmm_min.out
...
mpirun -machinefile $TMP/machines -np 4 -npernode 4 --map-by slot:pe=${QM_NUM_THREADS} $GENESIS genesis_qmmm_vib.inp >& genesis_qmmm_vib.out
